...
function movePaddle() {
  // Every time draw  on the canvas with can re-draw with
  // certain element
  ...
  if (paddle.x < 0) {
    // 0 from the x-axes  and this is for the borders detection
    paddle.x = 0;
  }
}
...
function moveBall() {
  ...
  if (ball.y + ball.size > canvas.height || ball.y - ball.size < 0) {
    ...
    ball.dy *= -1; // the reason we are doing this is to reverse the ball to go the
  ...
  // Paddle collision
  if (
    ball.x - ball.size > paddle.x && // Always to in consideration the object
  ...
  bricks.forEach((column) => {
    ...
    column.forEach((brick) => {
      if (brick.visible) {
        // make sure the bricks are visible
        if (
...
function increaseScore() {
  ...
  if (score % (brickColumnCount * brickRowCount) === 0) {
    // check the if there are no bricks if true redraw the bricks wall.
...
function showAllBricks() {
  ...
  bricks.forEach((column) => {
    // for column set visible to true
