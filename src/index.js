...
for (let i = 0; i < brickRowCount; i++) { // loop through array row
  ...
  for (let j = 0; j < brickColumnCount; j++) { // loop through array column
    const x = i * (brickInfo.w + brickInfo.padding) +
              brickInfo.offsetX; // i is the row iteration for each brick
    const y = j * (brickInfo.h + brickInfo.padding) +
              brickInfo.offsetY; // we are lopping and getting position
    bricks[i][j] = {
      x,
      y,
      ...brickInfo
    }; // copy and take the array 2D and give it the values of x,y with the
       // object values
  }
}
...
function drawBall() {
  ...
  ctx.arc(ball.x, ball.y, ball.size, 0,
          Math.PI * 2);      // Draw an arc to build a circle
...
function drawBricks() {
  bricks.forEach(column => { // For columns
    column.forEach(brick => {
      ...
      ctx.fillStyle =
          brick.visible ? '#0095dd' : 'transparent'; // This will be conditional
      ctx.fill();
      ctx.closePath();
    });
  });
...
function movePaddle() { // Every time draw  on the canvas with can re-draw with
                        // certain element
  paddle.x +=
      paddle.dx; // paddle will not move until we use the keyboards events
  ...
  if (paddle.x <
      0) { // 0 from the x-axes  and this is for the borders detection
    paddle.x = 0;
  }
}
...
function moveBall() {
  ...
  if (ball.x + ball.size > canvas.width ||
      ball.x - ball.size < 0) { // right and left walls
    ball.dx *= -1; // ball.dx = ball.dx * -1 the reason we are doing this is to
                   // reverse the ball to go the other wall and bounce back
  }

  // Wall collision (top/bottom)
  if (ball.y + ball.size > canvas.height ||
      ball.y - ball.size < 0) { // top and bottom walls
    ball.dy *=
        -1; // the reason we are doing this is to reverse the ball to go the
            // other wall and bounce back and not go outside the canvas
  }

  // console.log(ball.x, ball.y);

  // Paddle collision
  if (ball.x - ball.size > paddle.x && // Always to in consideration the object
                                       // size of ball and this check the left
    ...
    column.forEach(brick => {
      if (brick.visible) { // make sure the bricks are visible
        if (
          ...
          brick.visible =
              false; // once bounce off the brick make it visible false
...
function increaseScore() {
  ...
  if (score % (brickColumnCount * brickRowCount) ===
      0) { // check the if there are no bricks if true redraw the bricks wall.
           // score of 45 % 45 bricks becomes 0
...
function keyDown(e) {
  ...
  if (e.key === 'Right' ||
      e.key === 'ArrowRight') { //  so cases it refers to right
